...
Cti.adv_diploma_it = {
  reading_json: function() {
    ...
    priv.set_sub_lessons = function(sub_lessons, element) {
      element.find('li').last().append('<ul class="sub-lessons"></ul>')
      for(var idx in sub_lessons) {
        $('ul.sub-lessons').last().append('<li>' + sub_lessons[idx].text +'</li>')
      }
    };

    priv.set_unit_lessons = function(lessons) {
      $('#units li').last().append('<ul class="hide"></ul>')
      for(var idx in lessons) {
        $('#units ul.hide').last().append('<li>' + lessons[idx].name + ' ('+ Object.keys(lessons[idx].pc).length +') </li>')
        priv.set_sub_lessons(lessons[idx].pc, $('#units ul.hide').last());
      }
    };

    priv.set_course_units = function() {
      for(var idx in pub.data.units) {
        $('#units').append('<li>' + pub.data.units[idx].name + '<strong> ' + pub.data.units[idx].code + '</strong> ('+ Object.keys(pub.data.units[idx].EPC).length +') </li>')
        priv.set_unit_lessons(pub.data.units[idx].EPC);
      }
    };
